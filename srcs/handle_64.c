...
void insert_output_64(t_all *all, int nsyms, int symoff, int stroff)
{
	...
	i = 0;
	cmds = NULL;
	elements = (void *)all->ptr + symoff;
	all->stroff = (void *)all->ptr + stroff;
	while (i < nsyms)
	{
...
void handle_64(t_all *all)
{
	...
	all->cpu = 64;
	header = (struct mach_header_64 *) all->ptr;
	all->ncmds = header->ncmds;
	i = 0;
	lc = (void *)all->ptr + sizeof(*header);
	while (i < all->ncmds)
	{
...
char	*handle_64_findsec(t_all *all, struct nlist_64 *element)
{
	...
	i = -1;
	all->res = NULL;
	mach_header = (struct mach_header_64 *)((uintptr_t)all->ptr);
