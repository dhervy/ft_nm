...
void						find_type_64(t_all *all, t_cmd *cmd,\
							struct nlist_64 *element);
void						display_name(t_all *all);
void						nm(t_all *all);
int							check_corrompu(t_all *all, void *ptr);
char						*all_type(char *segname,\
							char *sectname, uint8_t n_type);
void						handle_64(t_all *all);
char						*handle_64_findsec(t_all *all,\
							struct nlist_64 *element);
...
void						init_add_list(t_cmd **begin,\
							t_cmd **tmp, t_cmd **tmp2, t_cmd **tmp3);
int							shearch_same_name(t_cmd **begin, t_cmd *this);
void						sort_lst(t_cmd **begin, t_cmd **tmp,\
							t_cmd **tmp2, t_cmd **tmp3);
void						sort_lst2(t_cmd **tmp, t_cmd **tmp2);
void						add_list_64(t_cmd **begin,\
							struct nlist_64 *nlist, t_all *all);
void						handle_32(t_all *all);
void						find_type_32(t_all *all, t_cmd *cmd,\
							struct nlist *element);
void						add_list_32(t_cmd **begin,\
							struct nlist *nlist, t_all *all);
char						*handle_32_findsec(t_all *all,\
							struct nlist *element);
uint32_t					swap32(uint32_t a);
void						handle_32_cigam(t_all *all);
void						add_list_32_cigam(t_cmd **begin,\
							struct nlist *nlist, t_all *all);
void						find_type_32_cigam(t_all *all, t_cmd *cmd,\
							struct nlist *element);
char						*handle_32_cigam_findsec(t_all *all,\
							struct nlist *element);
...
void						affi_otool_32_cigam(t_all *all,\
							struct section *section);
void						handle_32_cigam(t_all *all);
int							find_my_arch_norm(t_all *all);
void						print_name(t_all *all);
int							handle_fat_32_init(t_all *all,\
							struct fat_header **header, int *nbheader);
